...
export default function ContactDetailsForm({
  ...
  const updateContactSection = (
    ...
    setFormData((prev) => ({
      ...
      contacts: prev.contacts.map((contact, index) =>
        index === contactIndex ? { ...contact, ...updates } : contact
      ),
    }));
  };
    ...
    setFormData((prev) => ({
      ...
      contacts: prev.contacts.map((contact, index) =>
        index === contactIndex
          ? {
              ...contact,
              items: contact.items.map((item, j) =>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const invalidContacts = formData.contacts.filter(
      (contact) =>
    ...
    if (invalidContacts.length > 0) {
      toast.error(
        "All contact sections must have a title and at least one contact item"
      );
      setIsLoading(false);
      return;
    }
                      ...
                      <CardHeader
                        ...
                        subheader={
                          <Typography variant="body2" color="text.secondary">
                            {contact.items.filter((item) => item.trim()).length}{" "}
                            item
                            {contact.items.filter((item) => item.trim())
                              .length !== 1
                              ? "s"
                              : ""}
                          </Typography>
