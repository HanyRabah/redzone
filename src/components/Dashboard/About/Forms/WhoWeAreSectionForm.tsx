...
export default function WhoWeAreSectionForm({
  initialData,
}: WhoWeAreSectionFormProps) {
    ...
    return {
      ...
      extras: Array.isArray(initialData.extras)
        ? initialData.extras.map((extra) => ({
            title:
              typeof extra === "object" && extra !== null && "title" in extra
                ? String(extra.title)
                : "",
            list:
              typeof extra === "object" &&
  ...
  const updateExtra = (
    ...
    setFormData((prev) => ({
      ...
      extras: prev.extras.map((extra, i) =>
        i === index
          ? { ...JSON.parse(JSON.stringify(extra)), [field]: value }
          : extra
      ),
    }));
  };
      ...
      extras: prev.extras.map((extra, i) =>
        i === extraIndex
          ? {
              ...JSON.parse(JSON.stringify(extra)),
              list: [
                ...JSON.parse(JSON.stringify(extra)).list.filter(
                  (_, j) => j !== listIndex
                ),
              ],
            }
      ...
      extras: prev.extras.map((extra, i) =>
        i === extraIndex
          ? {
              ...JSON.parse(JSON.stringify(extra)),
              list: [
                ...JSON.parse(JSON.stringify(extra)).list.map((item, j) =>
                  j === listIndex ? value : item
                ),
              ],
            }
          : extra
        ...
        title={
          <Box sx={{ m: 1 }}>
            ...
            <Typography variant="caption" sx={{ color: "text.secondary" }}>
              Configure who we are section with title, description, and extras
            </Typography>
          </Box>
        }
      />
              ...
              <CardContent sx={{ p: 4 }}>
                <Stack spacing={3}>
                  <TextField
                    ...
                    helperText={
                      !formData.title.trim() ? "Section title is required" : ""
                    }
                    InputProps={{
                      startAdornment: (
                    ...
                    {formData.extras.map((extra, extraIndex) => {
                      ...
                      return (
                        <Box key={extraIndex}>
                              ...
                              >
                                ...
                                <Box sx={{ flex: 1 }}>
                                  <Typography
                                    ...
                                    sx={{ fontWeight: 600 }}
                                  >
                                    {JSON.parse(JSON.stringify(extra)).title ||
                                      `Section ${extraIndex + 1}`}
                                  </Typography>
                                  <Typography
                                    variant="body2"
                                    color="text.secondary"
                            ...
                            <AccordionDetails sx={{ p: 4 }}>
                              <Stack spacing={3}>
                                ...
                                <Box>
                                  ...
                                  <Stack spacing={2}>
                                    {JSON.parse(JSON.stringify(extra)).list.map(
                                      (item, listIndex) => (
                                        <Box
                                          key={listIndex}
                                          sx={{
                          ...
                          {extraIndex < formData.extras.length - 1 && (
                            <Divider />
                          )}
                        </Box>
                      );
              ...
              >
                <Box>
                  <Typography variant="body2" color="text.secondary">
                    {initialData ? "Update your" : "Create a new"} Who We Are
                    section
                  </Typography>
